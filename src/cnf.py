...
#
# 04.02.2025  Abhoy Kole
#   - Added debug comments around intermediate CNF/qCNF outputs for
#     troubleshooting generated clauses.
#   - Updated clean_qcnf_buffer_clauses() to remove only four-literal
#     qCNF buffer clauses where the two encoded literals are identical.
#   - Replaced the previous broad qCNF buffer-removal condition:
#       abs(c[2]) != abs(c[3])
#     with the safer condition:
#       not (len(c) == 4 and abs(c[2]) == abs(c[3]))
#   - Preserved qDIMACS output generation for miter-based equivalence
#     checking.
#   - Preserved DIMACS output generation and Lingeling-based equivalence
#     checking for classical CNF.
# 17.12.2025  Abhoy Kole
#   - Optimized DIMACS/qDIMACS literal counting in to_dimacs().
#   - Excluded qDIMACS ESOP markers from variable counting:
#       0   -> '@' XOR/ESOP separator
#       0.1 -> '!' constant-one marker
#   - Replaced nested buffer-clause cleanup loops in
#     clean_cnf_buffer_clauses() and clean_qcnf_buffer_clauses().
#   - Added replacement-map based cleanup for copied miter input buffers.
#   - Reduced repeated full-clause-list scans during buffer substitution.
#   - Removed redundant CNF/qCNF buffer clauses after substitution.
#   - Improved runtime for large miter generation, especially 32-bit
#     adder and multiplier benchmark circuits.
#
# -------------------------------------------------------------------------
import os
import sys
...
def to_dimacs(cnf, filename, inputs_1=None, miter_literal_map=None, qdimacs=False):
    ...
    for output in cnf:
        ...
        # Remap literals to sequential indices,
        # since buffer nodes might have introduced holes.
        map_index = 1
        lit_map = {0: 0}
        for clause in output:
            for lit in clause:
                if not (isinstance(lit, int) and lit != 0):
                    continue
                key = abs(lit)
                if key not in lit_map:
                    lit_map[key] = map_index
                    lit_map[-key] = -map_index
                    map_index += 1
        ...
        with open(filename, "w") as f:
            ...
            for i in inputs_1:
                f.write("c %s -> %d\n" % (i.name, lit_map[miter_literal_map[i]]))
...
def clean_cnf_buffer_clauses(miter_cnf, miter_literal_map, inputs_1, inputs_2):
    ...
    inp_1_lits = {miter_literal_map[i] for i in inputs_1}
    inp_2_lits = {miter_literal_map[i] for i in inputs_2}
    ...
    for clause in miter_cnf:
        ...
        if abs_a in inp_1_lits and abs_b in inp_2_lits:
            replacements[abs_b] = abs_a

        elif abs_b in inp_1_lits and abs_a in inp_2_lits:
            replacements[abs_a] = abs_b
    ...
    for clause in miter_cnf:
        for idx, lit in enumerate(clause):
            ...
            if abs_lit in replacements:
                new_lit = replacements[abs_lit]
                clause[idx] = new_lit if lit > 0 else -new_lit
...
def clean_qcnf_buffer_clauses(miter_qcnf, miter_literal_map, inputs_1, inputs_2):
    ...
    for clause in miter_qcnf:
        ...
        #                if -abs(inp1) in c:
        #                    c[c.index(-abs(inp1))] = -abs(inp2)
        if len(clause) not in (3, 4):
            continue

        real_lits = [lit for lit in clause if isinstance(lit, int) and lit != 0]

        if len(real_lits) != 2:
            continue

        lit_a, lit_b = real_lits
    ...
    miter_qcnf[:] = [
        ...
        if not (
            ...
            and isinstance(clause[3], int)
            and abs(clause[2]) == abs(clause[3])
        )
        and not (
            len(clause) == 3
            and isinstance(clause[0], int)
            and clause[1] == 0
            and isinstance(clause[2], int)
            and abs(clause[0]) == abs(clause[2])
        )
    ]

    # remove clauses of size 2 if they have the same literal i.e. they were buffer clauses before replacement
    #miter_qcnf[:] = [c for c in miter_qcnf if abs(c[2]) != abs(c[3])]
    #print("Here xx", miter_qcnf)
    #miter_qcnf[:] = [c for c in miter_qcnf if not (len(c) == 4 and abs(c[2]) == abs(c[3]))]
    #print("Here yy", miter_qcnf)
